...
#include "OpenXRHandTrackingCrossPlatformLiveLinkRemapAsset.h"
#include "OpenXRHandTrackingCrossPlatform.h"
#include "HeadMountedDisplayTypes.h"

#include "BonePose.h"

#define LOCTEXT_NAMESPACE "OpenXRHandTrackingCrossPlatformLiveLinkRemapAsset"

UOpenXRHandTrackingCrossPlatformLiveLinkRemapAsset::UOpenXRHandTrackingCrossPlatformLiveLinkRemapAsset(const FObjectInitializer& ObjectInitializer)
	: Super(ObjectInitializer)
{
}

#if WITH_EDITORONLY_DATA
void UOpenXRHandTrackingCrossPlatformLiveLinkRemapAsset::PostInitProperties()
{
	...
	if (HandTrackingBoneNameMap.Num() == 0)
	{
		...
		{
			FName BoneName = FOpenXRHandTrackingCrossPlatform::ParseEOpenXRHandKeypointEnumName(EnumPtr->GetNameByValue(Keypoint));
...
void UOpenXRHandTrackingCrossPlatformLiveLinkRemapAsset::BuildPoseFromAnimationData(float DeltaTime, const FLiveLinkSkeletonStaticData* InSkeletonData, const FLiveLinkAnimationFrameData* InFrameData, FCompactPose& OutPose)
{
...
FName UOpenXRHandTrackingCrossPlatformLiveLinkRemapAsset::GetRemappedBoneName(FName BoneName) const
{
...
FTransform UOpenXRHandTrackingCrossPlatformLiveLinkRemapAsset::GetRetargetedTransform(const FLiveLinkAnimationFrameData* InFrameData, int TransformIndex) const
{
	check(InFrameData);
